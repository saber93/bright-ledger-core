...
export const Route = createFileRoute("/api/communications/batch-preview")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const result =
            kind === "statements"
              ? await previewStatementBatch({
                  companyId,
                  search: typeof body.search === "string" ? body.search : null,
                })
              : await previewReminderBatch({
                  companyId,
                  search: typeof body.search === "string" ? body.search : null,
          ...
          return jsonResponse(
            {
              error:
                error instanceof Error
                  ? error.message
                  : "Failed to preview collections batch.",
            },
            500,
          );
        }
